  ...
  return (
    <nav className='w-full flex py-6 justify-between items-center navbar'>
      ...
      <div className='sm:hidden flex flex-1 justify-end items-center'>
        ...
        <div className={`${toggle ? 'flex' : 'hidden'} absolute top-20 right-0 mx-4 my-2 min-w-[140px] rounded-xl sidebar p-6 bg-black-gradient`}>
          <ul className='list-none flex items-center justify-end flex-col'>
            {navLinks.map((nav,index)=>(
              <li className={`font-poppins font-normal cursor-pointer text-[16px] text-white ${index === navLinks.length - 1 ? 'mb-0' : 'mb-4'}`} key={nav.id}>
                <a href={`#${nav.id}`}>{nav.title}</a>
              </li>
            ))}
          </ul>
